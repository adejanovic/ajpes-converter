...
import os, re, shutil, tempfile, uuid, subprocess, calendar, datetime
...
def validate(result):
    if not result.company.name and result.pdf_format not in ("GDZADRUGE", "OCR", "OCR_SCAN"):
        result.errors.append("Ime podjetja ni najdeno.")
    if not result.company.registration_number and result.pdf_format not in ("UPR", "GDZADRUGE", "OCR", "OCR_SCAN", "POROCILO_GD", "VIZIJA"):
    ...
    if any("AOP Konto" in l or "AOP" == l.strip()[:3] for l in lines[:8]):
        ...
        for line in lines[5:10]:
            if re.match(r'^\d{3}\s+', line.strip()):
                return "NAPOVED"
    # URADNI: čist uraden AJPES obrazec ("Oznaka za AOP" + podvojena koda npr. '001 B001')
    if "Oznaka" in first and "za AOP" in first and (
        re.search(r'\b\d{3}\s+[BP]\d{3}\b', first) or
        re.search(r'\b\d{3}\s+-?[\d.]+,\d{2}\s+-?[\d.]+,\d{2}', first)
    ):
        return "URADNI"
...
def parse_uradni_format(pages_text, tip_override=None, tip_subjekta_override=None):
    result = ParseResult(pdf_format="URADNI")
    full = "\n".join(pages_text)
    c = CompanyInfo()

    m = re.search(r'Matična številka:\s*(\d{7,10})', full)
    if m:
...
def _ocr_scan_page_entries(im, row_tol=8):
    width = im.size[0]
    data = pytesseract.image_to_data(im, lang='slv+eng', output_type=pytesseract.Output.DICT)
    n = len(data['text'])
    words = []
    for i in range(n):
        t = data['text'][i].strip()
    ...
    aop_x = (_OCR_SCAN_COL_FRACS["aop"][0]*width, _OCR_SCAN_COL_FRACS["aop"][1]*width)
    cur_x = (_OCR_SCAN_COL_FRACS["current"][0]*width, _OCR_SCAN_COL_FRACS["current"][1]*width)
    prev_x = (_OCR_SCAN_COL_FRACS["previous"][0]*width, _OCR_SCAN_COL_FRACS["previous"][1]*width)
            ...
            if pi < 2:
                ...
                if pi == 0:
                    ...
                    for i in range(len(data['text'])):
                        t = data['text'][i].strip()
                        if t:
                            header_words.append({'text': t, 'left': data['left'][i], 'top': data['top'][i]})
            for code, vals in _ocr_scan_page_entries(im).items():
...
def parse_pdf_file(pdf_path, tip_override=None, tip_subjekta_override=None):
    ...
    if fmt == "JOLP":
        ...
        result = parse_uradni_format(pages_text, tip_override, tip_subjekta_override)
    elif fmt == "VIZIJA":
