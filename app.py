...
def validate(result):
    if not result.company.name and result.pdf_format != "GDZADRUGE":
        result.errors.append("Ime podjetja ni najdeno.")
    if not result.company.registration_number and result.pdf_format not in ("UPR", "GDZADRUGE"):
...
def detect_pdf_format(pages_text):
    ...
    if "Oznaka" in first and "za AOP" in first and re.search(r'\b\d{3}\s+[BP]\d{3}\b', first):
        return "URADNI"
...
# ── PARSER: URADNI format (čist uraden AJPES obrazec, "Oznaka za AOP" + podvojena koda B001/P110) ──
# Ta obrazec ima ČISTO besedilo (brez prekrivanja/kerning napak kot pri AOPCOL) - vsaka postavka
# se konča z uzorcem "AOP_KODA [B|P]AOP_KODA ZNESEK_TEKOČE ZNESEK_PRETEKLO", kar je zelo zanesljivo
# za iskanje neodvisno od preloma vrstic v opisu postavke.
_URADNI_ENTRY_RE = re.compile(r'(\d{3})\s+[BP](\d{3})\s+(-?[\d.,]+)\s+(-?[\d.,]+)')
...
def parse_uradni_format(pages_text, tip_override=None, tip_subjekta_override=None):
    ...
    for m in _URADNI_ENTRY_RE.finditer(full):
...
def _fill_combined_rows(ws, aop_data, combined_defs):
    ...
    filled = 0
    for gvaop, code_dobicek, code_izguba in combined_defs:
        row = gvaop_idx.get(gvaop)
        if not row:
            continue
        e1 = aop_data.get(code_dobicek)
        e2 = aop_data.get(code_izguba)
