...
def validate(result):
    if not result.company.name and result.pdf_format not in ("GDZADRUGE", "OCR", "OCR_SCAN", "AJPES_NATIVE_XLS", "OBRACUNI_MEDLETNI", "AJPES_MEDLETNI_1COL_XLSX"):
        result.errors.append("Ime podjetja ni najdeno.")
    if not result.company.registration_number and result.pdf_format not in ("UPR", "GDZADRUGE", "OCR", "OCR_SCAN", "POROCILO_GD", "VIZIJA", "ZAVOD_MEDLETNI", "AJPES_NATIVE_XLS", "URADNI", "OBRACUNI_MEDLETNI", "BS_IPI_AJPES", "AJPES_1COL", "VASCO_AJPES_1COL", "AJPES_ZAP", "PODATKI_AJPES", "AJPES_MEDLETNI_1COL_XLSX", "NOVA_VERZIJA"):
...
def _is_podatki_ajpes_format(pages_text):
    """Prepozna izvoz z naslovom 'PODATKI IZ BILANCE STANJA' oz. 'PODATKI IZ IZKAZA
    POSLOVNEGA IZIDA' (npr. TERMIT d.d.). Prva vrstica prve strani je naslov, druga
    vrstica ime družbe, tretja glava 'postavka AOP <datum>' (BS, en datum "na dan")
    ali 'postavka AOP <rimska_od>. - <rimska_do>. <leto>' (IPI, obdobje v rimskih
    številkah mesecev). Vsaka postavka je na svoji (po potrebi večvrstični) vrstici,
    ki se konča z uradno AOP kodo in ENIM samim zneskom - format nima primerjalnega
    stolpca za preteklo leto. Matične številke ne vsebuje."""
    first = pages_text[0] if pages_text else ""
    lines = [ln.strip() for ln in first.split('\n') if ln.strip()]
    if not lines:
        return False
    has_title = bool(re.match(r'^PODATKI\s+IZ\s+(BILANCE\s+STANJA|IZKAZA\s+POSLOVNEGA\s+IZIDA)\b', lines[0], re.I))
    has_header = any(re.match(r'^postavka\s+AOP\b', ln, re.I) for ln in lines[:5])
    return has_title and has_header

def _is_nova_verzija_format(pages_text):
    """Prepozna izvoz oblike 'Bilanca stanja - NOVA VERZIJA' / 'Izkaz poslovnega izida -
    NOVA VERZIJA' (npr. GAREX d.o.o.). Glava: 'Podjetje:'/'Sedež:'/'Davčna št.:'/
    'Za obdobje: DD.MM.YYYY-DD.MM.YYYY' (BS uporablja isti 'Za obdobje' razpon kot IPI,
    ne 'na dan'). Format NE vsebuje matične številke, samo davčno. Vsaka postavka je na
    svoji vrstici in se konča z uradno AOP kodo in ENIM samim zneskom (brez primerjalnega
    stolpca za preteklo leto)."""
    first = pages_text[0] if pages_text else ""
    lines = [ln.strip() for ln in first.split('\n') if ln.strip()]
    if not lines:
        return False
    has_title = bool(re.match(r'^(Bilanca\s+stanja|Izkaz\s+poslovnega\s+izida)\s*-\s*NOVA\s+VERZIJA', lines[0], re.I))
    has_podjetje = any(ln.startswith("Podjetje:") for ln in lines[:6])
    return has_title and has_podjetje
...
def detect_pdf_format(pages_text):
    ...
    if "DENAR" in first and "OBRAČUNI" in first:
        return "VIZIJA"
    # PODATKI_AJPES: naslov "PODATKI IZ BILANCE STANJA" / "PODATKI IZ IZKAZA POSLOVNEGA
    # IZIDA" + glava "postavka AOP ..." (npr. TERMIT d.d.) - preverimo ga zgodaj, ker je
    # naslov zelo specifičen in se ne prekriva z drugimi formati.
    if _is_podatki_ajpes_format(pages_text):
        return "PODATKI_AJPES"
    # AJPES_PORTAL_2COL: uraden izvoz s portala AJPES ("Podatki iz bilance stanja"/"Podatki
    # iz izkaza poslovnega izida"), glava "Ime poslovnega subjekta:"/"Matična številka:" +
    # OBA stolpca zneskov (tekočega in prejšnjega leta) - preverimo ga pred URADNI/AOPCOL,
    # ker so boldane vrstice v surovem besedilu podvojene 4x in bi jih "Oznaka"/"za AOP"
    # test za URADNI format spregledal (glej _is_ajpes_portal_2col_pdf).
    if _is_ajpes_portal_2col_pdf(pages_text):
        return "AJPES_PORTAL_2COL"
    # AJPES_LETNO_PORTAL: polni uraden AJPES "letno poročilo" izvoz z naslovom "PODATKI IZ
    # BILANCE STANJA"/"PODATKI IZ IZKAZA POSLOVNEGA IZIDA" (VELIKE ČRKE), ki se ne pojavi na
    # prvi (naslovni "Osnovni podatki") strani - zato preverjamo čez cel dokument. Postavimo
    # ga za AJPES_PORTAL_2COL (drugačna glava/naslov), a pred URADNI/AOPCOL, ki bi ju "Oznaka"
    # + "za AOP" na kasnejših straneh sicer lahko po nesreči prestregla.
    if _is_ajpes_letno_portal_pdf(pages_text):
        return "AJPES_LETNO_PORTAL"
    # NOVA_VERZIJA: naslov "Bilanca stanja - NOVA VERZIJA"/"Izkaz poslovnega izida - NOVA
    # VERZIJA" + glava "Podjetje:" (npr. GAREX d.o.o.) - zelo specifičen naslov, preverimo
    # ga zgodaj, preden bi ga kak splošnejši test (npr. AOPCOL) po nesreči prestregel.
    if _is_nova_verzija_format(pages_text):
        return "NOVA_VERZIJA"
...
# ── PARSER: uraden AJPES portal izvoz "Podatki iz letnega poročila" (.xlsx, en sam    ──
# list, npr. "lp2025_<matična>.xlsx"). Glava podjetja je v ločenem bloku vrstic 1-22   ──
# (label v stolpcu A, vrednost precej desno - do 10+ stolpcev stran zaradi širokih     ──
# spojenih celic za "Konto"/"Postavka"), nato sledi ena sama dolga tabela BS + IPI +    ──
# bilančnega dobička + dodatnih podatkov o tujini, vse pod eno glavo "Konto | Postavka  ──
# | Oznaka za AOP | Znesek" (ki se zaradi izvoza iz PDF-ja ponavlja vsakih ~35 vrstic). ──
# AOP koda je shranjena kot GOLO CELO ŠTEVILO (ne kot niz "001"), zato je splošni       ──
# _aop_from_cell (ki namenoma izloči številke <100, da se izogne headerjem 1,2,3,4,5)   ──
# za ta format neuporaben - filtriramo namesto tega po tem, da je vrednost v stolpcu    ──
# "Postavka" niz (besedilo), medtem ko je pri ponavljajočih se header/oštevilčevalnih   ──
# vrsticah tam bodisi niz "Postavka" bodisi golo število (2) - obojemu se izognemo.     ──
def _find_ajpes_lp_portal_sheet(wb):
    for sn in wb.sheetnames:
        ws = wb[sn]
        a1 = ws.cell(row=1, column=1).value
        if isinstance(a1, str) and "Podatki iz letnega poročila" in a1:
            return sn
    return None

def parse_ajpes_lp_portal_xlsx(xlsx_path, tip_override=None, tip_subjekta_override=None):
    wb = openpyxl.load_workbook(xlsx_path, data_only=True)
    sheet_name = _find_ajpes_lp_portal_sheet(wb)
    ws = wb[sheet_name]
    rows = [tuple(row) for row in ws.iter_rows(values_only=True)]
    result = ParseResult(pdf_format="AJPES_LP_PORTAL")
    c = CompanyInfo()

    def _label_value(label, max_right=16):
        target = label.strip().lower()
        for row in rows:
            if not row:
                continue
            v0 = row[0]
            if isinstance(v0, str) and v0.strip().rstrip(':').lower() == target:
                for j in range(1, min(len(row), max_right)):
                    v = row[j]
                    if v is not None and str(v).strip() != "":
                        return v
        return None

    maticna = _label_value("Matična številka")
    if maticna is not None:
        c.registration_number = str(int(maticna)) if isinstance(maticna, (int, float)) and not isinstance(maticna, bool) else re.sub(r'\D', '', str(maticna))
    davcna = _label_value("Davčna številka")
    if davcna is not None:
        c.tax_number = str(int(davcna)) if isinstance(davcna, (int, float)) and not isinstance(davcna, bool) else re.sub(r'\D', '', str(davcna))
    ime = _label_value("Ime poslovnega subjekta")
    if ime:
        c.name = str(ime).strip()

    vrsta = _label_value("Vrsta")
    vs = str(vrsta).strip().lower() if vrsta else ""
    if "samostojni podjetnik" in vs:
        c.tip_subjekta = "2"
    elif "zadruga" in vs:
        c.tip_subjekta = "3"
    elif "društvo" in vs or "drustvo" in vs:
        c.tip_subjekta = "4"
    else:
        c.tip_subjekta = "1"  # privzeto: gospodarska družba

    def _fmt_date(v):
        return f"{v.day}.{v.month}.{v.year}" if isinstance(v, datetime.datetime) else None

    c.period_from = _fmt_date(_label_value("od"))
    c.period_to = _fmt_date(_label_value("do"))
    if not c.period_to:
        title = rows[0][0] if rows and rows[0] else ""
        m = re.search(r'(\d{1,2}\.\d{1,2}\.\d{4})\s*-\s*(\d{1,2}\.\d{1,2}\.\d{4})', str(title))
        if m:
            c.period_from, c.period_to = m.group(1), m.group(2)
    ...
    # Dinamično poišči stolpca "Postavka" in "Oznaka za AOP" (namesto trdo kodiranih
    # indeksov), za robustnost, če AJPES kdaj rahlo spremeni širino stolpcev "Konto".
    postavka_col = aop_col = None
    for row in rows:
        for idx, v in enumerate(row):
            if isinstance(v, str):
                vs2 = v.strip().lower()
                if vs2 == "postavka" and postavka_col is None:
                    postavka_col = idx
                elif vs2 == "oznaka za aop" and aop_col is None:
                    aop_col = idx
        if postavka_col is not None and aop_col is not None:
            break
    if aop_col is None:
        aop_col = 11
    if postavka_col is None:
        postavka_col = 4
    cur_col, prev_col = aop_col + 2, aop_col + 4

    for row in rows:
        if len(row) <= max(aop_col, cur_col, prev_col, postavka_col):
            continue
        postavka_val = row[postavka_col]
        aop_val = row[aop_col]
        if not isinstance(postavka_val, str) or not postavka_val.strip():
            continue
        if not isinstance(aop_val, (int, float)) or isinstance(aop_val, bool):
            continue
        if not float(aop_val).is_integer():
            continue
        n = int(aop_val)
        if not (1 <= n <= 499):
            continue
        code = str(n).zfill(3)
        cur = row[cur_col] if isinstance(row[cur_col], (int, float)) and not isinstance(row[cur_col], bool) else None
        prev = row[prev_col] if isinstance(row[prev_col], (int, float)) and not isinstance(row[prev_col], bool) else None
        if cur is None and prev is None:
            continue
        result.aop_data[code] = AopEntry(aop=code, current_year=cur, previous_year=prev)

    if not result.aop_data:
        result.warnings.append("V datoteki ni bilo mogoče najti nobene AOP postavke.")
...
# ── PARSER: AJPES medletni "1-stolpčni" .xlsx izvoz (listi BS/IPI, naslovi "Podatki iz  ──
# bilance stanja" / "Podatki iz izkaza poslovnega izida", stolpci Konto/Postavka/Oznaka  ──
# za AOP/Znesek - SAMO "Tekočega leta", brez "Prejšnjega leta"). Format ne vsebuje niti   ──
# imena podjetja niti matične številke (uporabnik ju vpiše ročno, enako kot pri           ──
# AJPES_NATIVE_XLS). Obdobje NUJNO beremo neposredno iz naslovnih vrstic obeh listov      ──
# ("na dan DD.MM.YYYY" / "v obdobju - DD.MM.YYYY-DD.MM.YYYY"), namesto da bi ga           ──
# izpeljevali (_infer_period_from), ker ta pri medletnih (ne-31.12.) datumih napačno       ──
# domneva NESTANDARDNO poslovno leto in izračuna napačen začetek obdobja.
def _is_ajpes_medletni_1col_xlsx(wb):
    for sn in wb.sheetnames:
        ws = wb[sn]
        for r in range(1, 3):
            v = ws.cell(row=r, column=1).value
            if isinstance(v, str) and (
                v.strip().startswith("Podatki iz bilance stanja")
                or v.strip().startswith("Podatki iz izkaza poslovnega izida")
            ):
                return True
    return False

def parse_ajpes_medletni_1col_xlsx(xlsx_path, tip_override=None, tip_subjekta_override=None):
    wb = openpyxl.load_workbook(xlsx_path, data_only=True)
    result = ParseResult(pdf_format="AJPES_MEDLETNI_1COL_XLSX")
    c = CompanyInfo()
    ...
    for sn in wb.sheetnames:
        ...
        for r in range(1, 3):
            ...
            if not isinstance(v, str):
                continue
            m = re.search(r'na dan\s+(\d{1,2}\.\d{1,2}\.\d{4})', v, re.I)
            if m and not c.period_to:
                c.period_to = m.group(1)
            m2 = re.search(r'(\d{1,2}\.\d{1,2}\.\d{4})\s*-\s*(\d{1,2}\.\d{1,2}\.\d{4})', v)
            if m2:
                c.period_from, c.period_to = m2.group(1), m2.group(2)

    if c.period_to and not c.period_from:
        leto = c.period_to.split('.')[-1]
        c.period_from = f"1.1.{leto}"
    ...
    for sn in wb.sheetnames:
        ...
        for row in ws.iter_rows(values_only=True):
            if len(row) < 4:
                continue
            postavka_val, aop_val, znesek_val = row[1], row[2], row[3]
            if not isinstance(postavka_val, str) or not postavka_val.strip():
                continue
            if not isinstance(aop_val, str):
                continue
            m = re.fullmatch(r'(\d{3})([a-z])?', aop_val.strip(), re.I)
            if not m:
                continue
            n = int(m.group(1))
            if not (1 <= n <= 499):
                continue
            code = m.group(1) + (m.group(2).lower() if m.group(2) else "")
            if isinstance(znesek_val, (int, float)) and not isinstance(znesek_val, bool):
                cur = float(znesek_val)
            elif znesek_val is None:
                cur = 0.0
            else:
                continue
            result.aop_data[code] = AopEntry(aop=code, current_year=cur, previous_year=None)
...
def parse_xlsx_file(xlsx_path, tip_override=None, tip_subjekta_override=None):
    ...
    if _find_ajpes_lp_portal_sheet(wb_check):
        return parse_ajpes_lp_portal_xlsx(xlsx_path, tip_override, tip_subjekta_override)
    if _is_ajpes_medletni_1col_xlsx(wb_check):
        return parse_ajpes_medletni_1col_xlsx(xlsx_path, tip_override, tip_subjekta_override)
...
# ── PARSER: PODATKI_AJPES format ("PODATKI IZ BILANCE STANJA" / "PODATKI IZ IZKAZA
# POSLOVNEGA IZIDA", npr. TERMIT d.d.) - vsaka postavka se konča z uradno AOP kodo
# in ENIM samim zneskom (brez primerjalnega stolpca za preteklo leto). Ker so AOP
# kode že vsebovane neposredno v besedilu, jih beremo direktno (ni potrebe po
# ujemanju po besedilu postavke kot pri npr. OBRACUNI_MEDLETNI).
_PODATKI_AJPES_ROW_RE = re.compile(r'(\d{3})\s+(-?[\d.]+,\d{2})\s*$')
_ROMAN_MONTHS = {"I": 1, "II": 2, "III": 3, "IV": 4, "V": 5, "VI": 6,
                 "VII": 7, "VIII": 8, "IX": 9, "X": 10, "XI": 11, "XII": 12}

def parse_podatki_ajpes_format(pages_text, tip_override=None, tip_subjekta_override=None):
    result = ParseResult(pdf_format="PODATKI_AJPES")
    lines0 = [ln.strip() for ln in (pages_text[0].split('\n') if pages_text else []) if ln.strip()]
    c = CompanyInfo()
    is_ipi = bool(lines0) and bool(re.match(
        r'^PODATKI\s+IZ\s+IZKAZA\s+POSLOVNEGA\s+IZIDA\b', lines0[0], re.I))
    if len(lines0) > 1:
        c.name = lines0[1].strip()

    header_line = next((ln for ln in lines0[:5] if re.match(r'^postavka\s+AOP\b', ln, re.I)), "")
    if is_ipi:
        m = re.search(r'postavka\s+AOP\s+([IVXLCDM]+)\.\s*-\s*([IVXLCDM]+)\.\s*(\d{4})', header_line, re.I)
        if m:
            start_month = _ROMAN_MONTHS.get(m.group(1).upper())
            end_month = _ROMAN_MONTHS.get(m.group(2).upper())
            year = int(m.group(3))
            if start_month:
                c.period_from = f"1.{start_month}.{year}"
            if end_month:
                last_day = calendar.monthrange(year, end_month)[1]
                c.period_to = f"{last_day}.{end_month}.{year}"
    else:
        m = re.search(r'postavka\s+AOP\s+(\d{1,2})\.(\d{1,2})\.(\d{4})', header_line)
        if m:
            d, mo, y = m.groups()
            c.period_to = f"{d}.{mo}.{y}"
            c.period_from = f"1.1.{y}"
    ...
    if not result.company.registration_number:
        result.warnings.append(
            "Ta format (Podatki iz bilance stanja/izkaza poslovnega izida) ne vsebuje "
            "matične številke — vpiši jo ročno, če jo banka zahteva."
        )
    ...
    for page_text in pages_text:
        for line in page_text.split('\n'):
            ...
            if not line:
                continue
            m = _PODATKI_AJPES_ROW_RE.search(line)
            if not m:
                continue
            aop = m.group(1)
            if aop not in BS_AOP and aop not in IPI_AOP:
                continue
            if aop in result.aop_data:
                continue
            current = parse_si(m.group(2))
            if current is not None:
                result.aop_data[aop] = AopEntry(aop=aop, current_year=current, previous_year=None)

    if not result.aop_data:
        result.warnings.append(
            "Format 'Podatki iz bilance stanja/izkaza poslovnega izida' ni bil prepoznan v tej datoteki.")

    return validate(result)


# ── PARSER: NOVA_VERZIJA format ("Bilanca stanja - NOVA VERZIJA" / "Izkaz poslovnega izida
# - NOVA VERZIJA", npr. GAREX d.o.o.). Glava: "Podjetje:"/"Sedež:"/"Davčna št.:"/"Za
# obdobje: DD.MM.YYYY-DD.MM.YYYY" (isti razpon uporablja tako BS kot IPI - BS nima ločenega
# "na dan"). Format NE vsebuje matične številke (samo davčno) - uporabnik jo mora dopolniti
# ročno. Vsaka postavka je na svoji vrstici in se konča z uradno AOP kodo in ENIM samim
# zneskom (brez primerjalnega stolpca za preteklo leto), zato je regex enak kot pri
# PODATKI_AJPES zgoraj.
_NOVA_VERZIJA_ROW_RE = re.compile(r'(\d{3})\s+(-?[\d.]+,\d{2})\s*$')

def parse_nova_verzija_format(pages_text, tip_override=None, tip_subjekta_override=None):
    result = ParseResult(pdf_format="NOVA_VERZIJA")
    full = "\n".join(pages_text)
    c = CompanyInfo()

    m = re.search(r'^Podjetje:\s*(.+)$', full, re.M)
    if m:
        c.name = m.group(1).strip()

    m = re.search(r'Davčna št\.:\s*(\d+)', full)
    if m:
        c.tax_number = m.group(1).strip()

    m = re.search(r'Za obdobje:\s*(\d{1,2}\.\d{1,2}\.\d{4})\s*-\s*(\d{1,2}\.\d{1,2}\.\d{4})', full)
    if m:
        c.period_from = m.group(1).strip()
        c.period_to = m.group(2).strip()
    ...
    if not result.company.registration_number:
        result.warnings.append(
            "Format 'NOVA VERZIJA' ne vsebuje matične številke (samo davčno) — vpiši jo "
            "ročno v celico D4 pred oddajo."
        )
    ...
    for page_text in pages_text:
        for line in page_text.split('\n'):
            ...
            if not line:
                continue
            m = _NOVA_VERZIJA_ROW_RE.search(line)
            if not m:
                continue
            aop = m.group(1)
            if aop not in BS_AOP and aop not in IPI_AOP:
    ...
    if not result.aop_data:
        result.warnings.append(
            "Format 'Bilanca stanja/Izkaz poslovnega izida - NOVA VERZIJA' ni bil prepoznan "
            "v tej datoteki.")

    return validate(result)


# ── PARSER: AJPES_PORTAL_2COL format ("Podatki iz bilance stanja" / "Podatki iz izkaza
# poslovnega izida", uraden izvoz s portala AJPES/eObrazci, npr. NUX d.o.o.). Glava ima
# "Ime poslovnega subjekta:" / "Matična številka:" / "Sedež poslovnega subjekta:" /
# "Davčna številka:" v dvostolpčni postavitvi (oznaka in vrednost NISTA na isti vrstici -
# obe oznaki sta na eni vrstici, obe pripadajoči vrednosti pa skupaj na NASLEDNJI vrstici,
# npr. "NUX d.o.o. 9372636000"), tabela pa ima OBA stolpca zneskov (tekočega IN
# prejšnjega leta), za razliko od sorodnega PODATKI_AJPES formata zgoraj.
#
# POSEBNOST: PDF simulira boldano pisavo (naslovi, seštevkovne/glavne postavke kot
# SREDSTVA, A./B./C..., I./II...) tako, da v besedilnem sloju vsak znak ponovi 4x
# (npr. "SREDSTVA" postane "SSSSRRRREEEEDDDDSSSSTTTTVVVVAAAA"). Navadne (nepoudarjene)
# vrstice ostanejo nespremenjene. _dequad_text to razveljavi na nivoju besed (ločenih s
# presledki): če je beseda dolga za večkratnik 4 in je vsaka zaporedna četverica znakov
# enaka, jo strne na en sam znak na četverico - zato ga pokličemo pred vsakim branjem
# glave ali AOP vrstic.
def _dequad_token(tok):
    n = len(tok)
    if n == 0 or n % 4 != 0:
        return tok
    chunks = [tok[i:i+4] for i in range(0, n, 4)]
    if all(len(set(ch)) == 1 for ch in chunks):
        return ''.join(ch[0] for ch in chunks)
    return tok

def _dequad_text(text):
    return '\n'.join(' '.join(_dequad_token(t) for t in line.split(' ')) for line in text.split('\n'))

def _is_ajpes_portal_2col_pdf(pages_text):
    if not pages_text:
        return False
    first = _dequad_text(pages_text[0])
    has_subject = "Ime poslovnega subjekta" in first
    has_title = bool(re.search(r'Podatki iz (bilance stanja|izkaza poslovnega izida)', first, re.I))
    has_cols = ("Oznaka" in first and "za AOP" in first
                and "Tekočega leta" in first and "Prejšnjega leta" in first)
    return has_subject and has_title and has_cols

_AJPES_PORTAL_ROW_RE_DEC = re.compile(r'(\d{3})\s+(-?[\d.]+,\d{2})\s+(-?[\d.]+,\d{2})\s*$')
_AJPES_PORTAL_ROW_RE_INT = re.compile(r'(\d{3})\s+(-?\d+)\s+(-?\d+)\s*$')

def _ajpes_portal_extract_header_pair(full, label):
    """Za vrstico z oznako (npr. 'Ime poslovnega subjekta:') poišče NASLEDNJO vrstico, kjer
    sta ime/naslov IN pripadajoča številka (matična/davčna) združena na isti vrstici -
    številko (7-10 mestno, na koncu vrstice) odrežemo od besedila pred njo."""
    lines = full.split('\n')
    for i, ln in enumerate(lines):
        if ln.strip().startswith(label):
            if i + 1 < len(lines):
                val_line = lines[i+1].strip()
                m = re.match(r'^(.*\S)\s+(\d{7,10})\s*$', val_line)
                if m:
                    return m.group(1).strip(), m.group(2).strip()
                return val_line.strip(), None
            break
    return None, None

def parse_ajpes_portal_2col_pdf(pages_text, tip_override=None, tip_subjekta_override=None):
    result = ParseResult(pdf_format="AJPES_PORTAL_2COL")
    full = _dequad_text("\n".join(pages_text))
    c = CompanyInfo()

    name, reg = _ajpes_portal_extract_header_pair(full, "Ime poslovnega subjekta")
    if name:
        c.name = name
    if reg:
        c.registration_number = reg
    _, tax = _ajpes_portal_extract_header_pair(full, "Sedež poslovnega subjekta")
    if tax:
        c.tax_number = tax

    is_ipi = bool(re.search(r'Podatki iz izkaza poslovnega izida', full, re.I))
    if is_ipi:
        m = re.search(r'v obdobju\s+(\d{1,2}\.\d{1,2}\.\d{4})\s*-\s*(\d{1,2}\.\d{1,2}\.\d{4})', full)
        if m:
            c.period_from = m.group(1).strip()
            c.period_to = m.group(2).strip()
    else:
    ...
    if not result.company.registration_number:
        result.warnings.append(
            "V formatu 'Podatki iz bilance stanja/izkaza poslovnega izida' (AJPES portal) "
            "matična številka ni bila prepoznana — preveri in po potrebi vpiši ročno v celico D4."
        )
    ...
    for line in full.split('\n'):
        ...
        if not line:
            continue
        m = _AJPES_PORTAL_ROW_RE_DEC.search(line)
        if not m:
            m = _AJPES_PORTAL_ROW_RE_INT.search(line)
        if not m:
            continue
        aop = m.group(1)
        if aop not in BS_AOP and aop not in IPI_AOP:
        ...
        if aop in result.aop_data:
            continue
        current = parse_si(m.group(2)) if ',' in m.group(2) else float(m.group(2))
        previous = parse_si(m.group(3)) if ',' in m.group(3) else float(m.group(3))
        if current is not None or previous is not None:
            result.aop_data[aop] = AopEntry(aop=aop, current_year=current, previous_year=previous)

    if not result.aop_data:
        result.warnings.append(
            "Format 'Podatki iz bilance stanja/izkaza poslovnega izida' (AJPES portal, 2 stolpca "
            "zneskov) ni bil prepoznan v tej datoteki.")

    return validate(result)


# ── PARSER: AJPES_LETNO_PORTAL format ("PODATKI IZ BILANCE STANJA" / "PODATKI IZ IZKAZA
# POSLOVNEGA IZIDA", VELIKE ČRKE - polni uraden AJPES "letno poročilo" izvoz, npr. AUTRAN
# d.o.o.). To je EN sam večstranski PDF, ki poleg BS in IPI vsebuje tudi naslovno stran
# "Podatki iz letnega poročila" (Osnovni podatki), izkaz bilančnega dobička/izgube (AOP
# 200-216, 302), dodatne podatke do tujine (AOP 217-284) in razkritja (AOP 303-307) - vse
# to izven BS_AOP/IPI_AOP razponov, zato se samodejno izloči brez posebne obravnave.
#
# Za razliko od AJPES_PORTAL_2COL zgoraj:
#  - naslov "PODATKI IZ BILANCE STANJA"/"PODATKI IZ IZKAZA POSLOVNEGA IZIDA" (in glava
#    tabele) se NE pojavi na prvi strani (ta je naslovna "Osnovni podatki"), zato je treba
#    prepoznavo in branje izvajati čez CELOTEN dokument, ne le prvo stran;
#  - glava ima obliko "Matična številka:NNNN\nIme poslovnega subjekta Davčna
#    številka:MMMM\n<NAZIV>" (matična/davčna pred nazivom, brez dvopičja za "Ime
#    poslovnega subjekta");
#  - naslovni datum ("na dan"/"v obdobju od") ima včasih presledke MED posameznimi znaki
#    številke (npr. "3 0 . 0 6 . 2 0 2 6") zaradi razmika pisave v naslovu - v podatkovnih
#    vrsticah tega pojava ni, zato ga razveljavimo samo pri branju datuma;
#  - PRIMERJALNI stolpec (prejšnje leto) je večinoma prazen - vsaka vrstica ima lahko EN
#    ali DVA zneska, zato najprej poskusimo dvo-vrednostni vzorec, nato eno-vrednostnega.
def _is_ajpes_letno_portal_pdf(pages_text):
    if not pages_text:
        return False
    full = "\n".join(pages_text)
    has_subject = "Ime poslovnega subjekta" in full
    has_title = bool(re.search(r'PODATKI\s+IZ\s+(BILANCE\s+STANJA|IZKAZA\s+POSLOVNEGA\s+IZIDA)', full))
    has_cols = ("Oznaka" in full and "za AOP" in full
                and "Tekočega leta" in full and "Prejšnjega leta" in full)
    return has_subject and has_title and has_cols

_AJPES_LETNO_ROW_RE_2VAL = re.compile(r'(\d{3})\s+(-?[\d.]+,\d{2})\s+(-?[\d.]+,\d{2})\s*$')
_AJPES_LETNO_ROW_RE_1VAL = re.compile(r'(\d{3})\s+(-?[\d.]+,\d{2})\s*$')

def parse_ajpes_letno_portal_pdf(pages_text, tip_override=None, tip_subjekta_override=None):
    result = ParseResult(pdf_format="AJPES_LETNO_PORTAL")
    full = "\n".join(pages_text)
    lines = full.split('\n')
    c = CompanyInfo()
    ...
    for i, ln in enumerate(lines):
        if ln.strip().startswith("Ime poslovnega subjekta"):
            for j in range(i + 1, min(i + 3, len(lines))):
                cand = lines[j].strip()
                if cand and not cand.startswith(("Sedež", "Davčna", "Matična", "Velikost", "Statusna")):
                    c.name = cand
                    break
            break
    ...
    for ln in lines:
        ls = ln.strip()
        if ls.startswith("na dan"):
            raw = re.sub(r'\s+', '', ls[len("na dan"):])
            if re.match(r'^\d{1,2}\.\d{1,2}\.\d{4}$', raw):
                c.period_to = raw
                c.period_from = f"1.1.{raw.split('.')[-1]}"
            break
        if ls.startswith("v obdobju od"):
            raw = re.sub(r'\s+', '', ls[len("v obdobju od"):])
            parts = raw.split('-')
            if len(parts) == 2:
                c.period_from, c.period_to = parts[0], parts[1]
            break
    ...
    if not result.company.registration_number:
        result.warnings.append(
            "V formatu 'PODATKI IZ BILANCE STANJA/IZKAZA POSLOVNEGA IZIDA' (AJPES letno "
            "poročilo) matična številka ni bila prepoznana — preveri in po potrebi vpiši "
            "ročno v celico D4."
        )
    ...
    for line in lines:
        ...
        if not line:
            continue
        m = _AJPES_LETNO_ROW_RE_2VAL.search(line)
        if m:
            aop = m.group(1)
            if (aop in BS_AOP or aop in IPI_AOP) and aop not in result.aop_data:
                current = parse_si(m.group(2))
                previous = parse_si(m.group(3))
                if current is not None or previous is not None:
                    result.aop_data[aop] = AopEntry(aop=aop, current_year=current, previous_year=previous)
            continue
        m = _AJPES_LETNO_ROW_RE_1VAL.search(line)
        if m:
            aop = m.group(1)
            if (aop in BS_AOP or aop in IPI_AOP) and aop not in result.aop_data:
                current = parse_si(m.group(2))
    ...
    if not result.aop_data:
        result.warnings.append(
            "Format 'PODATKI IZ BILANCE STANJA/IZKAZA POSLOVNEGA IZIDA' (AJPES letno "
            "poročilo) ni bil prepoznan v tej datoteki.")
...
def parse_pdf_file(pdf_path, tip_override=None, tip_subjekta_override=None):
    ...
    elif fmt == "PODATKI_AJPES":
        result = parse_podatki_ajpes_format(pages_text, tip_override, tip_subjekta_override)
    elif fmt == "NOVA_VERZIJA":
        result = parse_nova_verzija_format(pages_text, tip_override, tip_subjekta_override)
    elif fmt == "AJPES_PORTAL_2COL":
        result = parse_ajpes_portal_2col_pdf(pages_text, tip_override, tip_subjekta_override)
    elif fmt == "AJPES_LETNO_PORTAL":
        result = parse_ajpes_letno_portal_pdf(pages_text, tip_override, tip_subjekta_override)
    else:
...
def export_excel(result, output_path):
    ...
    Datumske celice dobijo pravi Excel DATE (Python date). Samo na treh datumskih celicah
    eksplicitno nastavimo kratek datum (dd.mm.yyyy); vseh drugih stilov in formatov ne spreminjamo.
    """
    ...
    # Datuma sta pravi Excel DATE vrednosti in morata biti prikazana v kratki obliki.
    # CBK uvoz pri teh celicah pričakuje kratek datum (npr. 31.05.2026), ne dolge oblike.
    ws.cell(11,4).value = _to_excel_date(c.period_to)
    ws.cell(11,4).number_format = "dd.mm.yyyy"
    # C8 ("Podatki iz bilance stanja" datum) je bilo prej treba vpisati ročno — zdaj ga
    # napolnimo samodejno z istim datumom kot D11 in prav tako prisilimo kratko obliko.
    ws.cell(8,3).value  = _to_excel_date(c.period_to)
    ws.cell(8,3).number_format = "dd.mm.yyyy"
    ...
    ws2.cell(11,4).number_format = "dd.mm.yyyy"
