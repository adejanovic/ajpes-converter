...
# Tip subjekta za template izbiro
TIP_SUBJEKTA_GD = "GD"        # Gospodarska družba → CBK template (obstoječi)
TIP_SUBJEKTA_DRUSTVO = "DR"   # Društvo → CBK template za društva (ko ga dobiš)
...
def parse_aop_format(pages_text, tip_override=None):
    ...
    result.subject_type = _detect_subject_type_from_text(full)
...
def _detect_xlsx_format(wb):
    """Zazna LP2025 format (A=konto, B=postavka, C=AOP, D=vrednost) vs JOLP (A=label, B=vrednost)."""
    for sheet_name in wb.sheetnames:
        ws = wb[sheet_name]
        for row in ws.iter_rows(min_row=10, max_row=20, values_only=True):
            if len(row) >= 4 and row[2] is not None:
                try:
                    code = int(str(row[2]).strip())
                    if 1 <= code <= 499 and isinstance(row[3], (int, float)):
                        return "LP2025"
                except: pass
    return "JOLP"
...
def parse_xlsx_file(xlsx_path, tip_override=None):
    ...
    all_cell_text = ""
    xlsx_fmt = _detect_xlsx_format(wb)
    ...
    for sheet_name in wb.sheetnames:
        ...
        for row in ws.iter_rows(values_only=True):
            ...
            if xlsx_fmt == "LP2025" and len(row) >= 4:
                # LP2025: A=konto, B=postavka, C=AOP, D=tek.leto, E=prej.leto
                a = str(row[0]).strip() if row[0] is not None else ""
                b = str(row[1]).strip() if row[1] is not None else ""
                c_aop = str(row[2]).strip() if row[2] is not None else ""
                d_val = row[3]
                e_val = row[4] if len(row) > 4 else None
                all_cell_text += a + " " + b + " "

                # Podatki podjetja - vrstica z imenom ima D=matična (10 cifer)
                if row[3] is not None and not c.registration_number:
                    d_str = str(row[3]).strip()
                    if d_str.isdigit() and len(d_str) == 10:
                        c.registration_number = d_str
                        if a and a not in ("Ime poslovnega subjekta","Sedež poslovnega subjekta",""):
                            c.name = a
                    elif d_str.isdigit() and len(d_str) == 8 and not c.tax_number:
                        c.tax_number = d_str

                # Obdobje
                for txt in [a, b]:
                    m = re.search(r'na dan\s+([\d]{1,2}\.[\d]{1,2}\.[\d]{4})', txt)
                    if m: c.period_to = m.group(1)
                    m = re.search(r'(\d{1,2}\.\d{1,2}\.\d{4})\s*-\s*(\d{1,2}\.\d{1,2}\.\d{4})', txt)
                    if m: c.period_from = m.group(1); c.period_to = m.group(2)

                # AOP vrednosti - normalna koda (001-310)
                try:
                    aop_int = int(c_aop)
                    if 1 <= aop_int <= 499 and isinstance(d_val, (int, float)):
                        aop_code = str(aop_int).zfill(3)
                        if aop_code not in result.aop_data:
                            result.aop_data[aop_code] = AopEntry(aop=aop_code,
                                current_year=float(d_val),
                                previous_year=float(e_val) if isinstance(e_val,(int,float)) else None)
                except: pass

                # AOP z a-sufiksom (npr. 056a)
                if re.match(r'\d{3}[a-z]', c_aop) and isinstance(d_val,(int,float)):
                    if c_aop not in result.aop_data:
                        result.aop_data[c_aop] = AopEntry(aop=c_aop,
                            current_year=float(d_val),
                            previous_year=float(e_val) if isinstance(e_val,(int,float)) else None)
            else:
                # JOLP format: A=label, B=tek.leto, C=prej.leto
                col0 = str(row[0]).strip() if row[0] is not None else ""
                col1 = row[1]; col2 = row[2] if len(row) > 2 else None
                if col0: all_cell_text += col0 + " "
    ...
    if (not c.period_from or len(c.period_from) < 6) and c.period_to:
        c.period_from = _infer_period_from(c.period_to)
    result.subject_type = _detect_subject_type_from_text(all_cell_text)
    result.company = finalize_company(c, tip_override)
...
def export_excel(result, output_path):
    # Izberi pravi template glede na tip subjekta
    subject_type = getattr(result, 'subject_type', 'GD')
    if subject_type == "DR" and TEMPLATE_DRUSTVO_PATH.exists():
        tmpl = TEMPLATE_DRUSTVO_PATH
    else:
        tmpl = TEMPLATE_PATH
        if subject_type == "DR":
            result.warnings.append(
                "Zaznano DRUŠTVO — template za društva še ni naložen. "
                "Uporabljen je standardni GD template."
            )
  ...
  <div class="card">
    ...
    <div id="app-section" style="display:none;">
      ...
      <div class="result-box" id="result-box">
        ...
        <div class="warn-note" id="warn-drustvo" style="display:none;background:#fef3c7;border-color:#fcd34d;color:#92400e;">&#9888; Zaznano <strong>DRUŠTVO</strong> — template za društva še ni naložen na strežniku. Ko ga prejmete, ga dodajte kot <code>template_drustvo.xlsx</code>.</div>
        </div>
      </div>
...
function filesSelected(fileList){
  for(let f of fileList){
    ...
    if(!ext.endsWith('.pdf')&&!ext.endsWith('.xlsx')){
      showError('Dovoljeni so samo PDF ali Excel (.xlsx) fajli.'); return;
    }
...
async function convert(){
  ...
  try{
    ...
    document.getElementById('r-subj').textContent=subj==='DR'?'Društvo / nevladna org.':'Gospodarska družba';
    ...
    if(subj==='DR'){document.getElementById('warn-drustvo').style.display='block';}
    else{document.getElementById('warn-drustvo').style.display='none';}
...
def convert():
    ...
    try:
        ...
        tip_label = {"7": "PRELIMINARNA", "8": "ZAKLJUCENA", "3": "REVIDIRANA"}.get(c.tip_bilance, "BILANCA")
        ...
        response.headers["Access-Control-Expose-Headers"] = \
            "X-Download-Name,X-Subject-Type,X-Company-Name,X-Registration,X-Period,X-Tip-Bilance,X-PDF-Format,X-BS-Filled,X-IPI-Filled"
        return response
