...
import os, re, shutil, tempfile, uuid, subprocess, calendar
...
def _classify_and_strip_item(s):
    """Vrne (nivo_oznake, besedilo_brez_oznake_in_oklepaja). Nivo je eden od
    'roman' (I./II./...), 'letter' (A./B./.../Č./...), 'arabic' (1./2a./...),
    'lower' (a)/b)/...) ali 'none' (brez vodilne oznake, npr. 'SREDSTVA').
    Nivo se uporabi, da se ista besedilna oznaka na RAZLIČNIH nivojih hierarhije
    (npr. skupinski seštevek 'B. Rezervacije' proti podpostavki '1. Rezervacije')
    ne pomeša med sabo."""
    s = s.strip()
    s = re.sub(r'\s*\(.*$', '', s)
    m = re.match(r'^([IVXLCDM]{1,6})\.\s*', s, flags=re.I)
    if m: return ('roman', s[m.end():].strip().lower())
    m = re.match(r'^([A-ZČŽŠ])\.\s*', s)
    if m: return ('letter', s[m.end():].strip().lower())
    m = re.match(r'^(\d{1,2}[a-zčžš]?)\.\s*', s, flags=re.I)
    if m: return ('arabic', s[m.end():].strip().lower())
    m = re.match(r'^([a-zčžš])\)\s*', s, flags=re.I)
    if m: return ('lower', s[m.end():].strip().lower())
...
def validate(result):
    ...
    if not result.company.registration_number and result.pdf_format != "UPR":
