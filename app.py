...
def validate(result):
    ...
    if not result.company.registration_number and result.pdf_format not in ("UPR", "GDZADRUGE", "OCR", "OCR_SCAN", "POROCILO_GD", "VIZIJA", "ZAVOD_MEDLETNI", "AJPES_NATIVE_XLS", "URADNI", "OBRACUNI_MEDLETNI", "BS_IPI_AJPES"):
...
def _native_uradni_xls_company(rows):
    ...
    for i, row in enumerate(rows[:20]):
        if not row or not isinstance(row[0], str):
            continue
        label = row[0].strip()
        nxt = rows[i + 1][0] if i + 1 < len(rows) and rows[i + 1] else None
...
def _ocr_preprocess(im, threshold=None):
...
def _ocr_scan_page_entries_percropped(page, dpi, col_fracs, header_row_words):
    ...
    quick = pytesseract.image_to_data(_ocr_preprocess(im_raw), lang='slv+eng',
    ...
    def _process(rt):
        ...
        proc = ImageOps.autocontrast(ImageOps.grayscale(crop_big), cutoff=1)
...
# ── OCR za skenirana LETNA POROČILA brez AOP stolpca ─────────────────────────
# Revidirana letna poročila so pogosto samo sken strani iz revizorjevega PDF-ja. V takem
# dokumentu postavke niso označene z AOP kodami, zato jih ni mogoče obdelati z zgornjim
# OCR_SCAN parserjem. Ta pot najprej pri nizki ločljivosti poišče samo tri strani računovodskih
# izkazov, nato pa jih pri višji ločljivosti prebere po nazivih postavk. S tem se izognemo
# večminutnemu OCR-u vseh strani in hkrati ohranimo sledljive, računovodsko preverljive izračune.

def _annual_normalize_label(text):
    """Normalizira slovenski OCR za varno primerjanje nazivov postavk."""
    s = unicodedata.normalize("NFKD", str(text or ""))
    s = ''.join(ch for ch in s if not unicodedata.combining(ch)).upper()
    # Pogoste zamenjave pri fotokopijah: č→é/e, š→8, l↔i in podobno.
    replacements = {
        "CGISTI": "CISTI", "GISTL": "CISTI", "GISTI": "CISTI", "GIST": "CIST",
        "EISTI": "CISTI",
        "DALGORO": "DOLGORO", "DELGORE": "DOLGORE", "DELGORO": "DOLGORO",
        "DOLGOREENE": "DOLGOROCNE", "DOLGOROINA": "DOLGOROCNA",
        "DOLGOROT": "DOLGOROC",
        "KRATKARO": "KRATKORO", "KRATKOROGN": "KRATKOROCN", "KRATKOROT": "KRATKOROC",
        "DOLGOROENE": "DOLGOROCNE",
        "DOLGOROENA": "DOLGOROCNA", "KRATKOROENE": "KRATKOROCNE",
        "KRATKOROENA": "KRATKOROCNA", "EASOVNE": "CASOVNE", "TASOVNE": "CASOVNE",
        "FINAN NI": "FINANCNI", "FINANGNI": "FINANCNI", "FINANTNI": "FINANCNI",
        "FINANTNE": "FINANCNE",
        "FINANINE": "FINANCNE", "FINANENT": "FINANCNI", "FINANEAVH": "FINANCNIH",
        "FINANENE": "FINANCNE",
        "FINANENI": "FINANCNI", "FINANNE": "FINANCNE",
        "DOBIEKA": "DOBICKA", "DOBITKA": "DOBICKA", "MENIENIH": "MENICNIH",
        "POSTOVNI": "POSLOVNI", "POSTOVNE": "POSLOVNE", "POSTOVAI": "POSLOVNI",
        "POSIOVNI": "POSLOVNI", "POSIOVNE": "POSLOVNE", "PASLOVNI": "POSLOVNI",
        "ABVEZNOSTI": "OBVEZNOSTI", "CBVEZNOSTI": "OBVEZNOSTI",
        "EBVEZNOSTI": "OBVEZNOSTI", "OBVEZNASTI": "OBVEZNOSTI",
        "SOCIAINIH": "SOCIALNIH", "POKOFNINSKIH": "POKOJNINSKIH",
        "ZEMIJI": "ZEMLJI", "ZEMLJISEA": "ZEMLJISCA", "OSNOYNA": "OSNOVNA",
        "GRADNYI": "GRADNJI", "PREMODENJSKE": "PREMOZENJSKE",
        "PREDUJMIZA": "PREDUJMI ZA", "DENAMASREDSTVA": "DENARNA SREDSTVA",
        "PROIZVEDI": "PROIZVODI", "TRGOVSKE BLAGO": "TRGOVSKO BLAGO",
        "PRENASENI": "PRENESENI", "DOBAVITELLEV": "DOBAVITELJEV", "PADLAGI": "PODLAGI",
        "TERJATVE DE DRUGIH": "TERJATVE DO DRUGIH", "DAVINA": "DAVCNA",
        "MATIENA": "MATICNA",
        "STEVITKA": "STEVILKA",
        "STRO8KI": "STROSKI", "STROFKI": "STROSKI", "STRODKI": "STROSKI",
        "STROGKI": "STROSKI", "STROGK": "STROSKI", "STROKI": "STROSKI",
        "STROBK": "STROSK", "PRINODK": "PRIHODK", "PRIHEDK": "PRIHODK",
        "EDHODKI": "ODHODKI", "ODHODKD": "ODHODKI",
        "OBRATUNSKEGA": "OBRACUNSKEGA", "OBYATUNSKEGA": "OBRACUNSKEGA",
        "PLAT": "PLAC",
    }
...
def _annual_compact(text):
    return _annual_normalize_label(text).replace(' ', '')


def _annual_ocr_page_text(page, dpi=120, psm=6):
    im = _ocr_preprocess(page.to_image(resolution=dpi).original)
    return pytesseract.image_to_string(im, lang='slv+eng', config=f'--psm {psm}')


def _probe_scanned_pdf(pdf_path):
    """Hitro določi, ali je sken AOP obrazec ali letno poročilo brez AOP oznak.

    OCR teče zaporedno in se ustavi takoj, ko so najdene strani sredstev, obveznosti in IPI.
    Besedila že pregledanih strani se vrnejo tudi za izluščenje glave podjetja.
    """
    probe = {
        "kind": "UNKNOWN", "page_texts": {},
        "bs_assets": None, "bs_liabilities": None, "ipi": None,
    }
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for pi, page in enumerate(pdf.pages):
            try:
                text = _annual_ocr_page_text(page, dpi=120, psm=6)
            except Exception:
                continue
            probe["page_texts"][pi] = text
            compact = _annual_compact(text)
            digits = len(re.findall(r'\d', text))

            # Skeniran AJPES obrazec z eksplicitnim AOP stolpcem: ohrani obstoječi parser.
            if ("OZNAKAZAAOP" in compact or
                    ("TEKOCEGALETA" in compact and "PREJSNJEGALETA" in compact)):
                probe["kind"] = "OCR_AOP"
                return probe

            if (probe["bs_assets"] is None and digits >= 20 and
                    "BILANCASTANJA" in compact and "SREDSTVA" in compact and
                    "DOLGORO" in compact and "KRATKORO" in compact and "ZALOGE" in compact):
                probe["bs_assets"] = pi
            if (probe["bs_liabilities"] is None and digits >= 20 and
                    "VIROVSREDSTEV" in compact and "KAPITAL" in compact and
                    "KRATKORO" in compact and "OBVEZNOST" in compact):
                probe["bs_liabilities"] = pi
            if (probe["ipi"] is None and digits >= 30 and
                    "IZKAZPOSLOVNEGAIZIDA" in compact and
                    "PRIHODKIODPRODAJE" in compact and "STRO" in compact and "DELA" in compact):
                probe["ipi"] = pi

            if all(probe[k] is not None for k in ("bs_assets", "bs_liabilities", "ipi")):
                probe["kind"] = "OCR_ANNUAL"
                break
    return probe


_ANNUAL_AMOUNT_TOKEN = r'(?:[-–—.]|\(?-?\d[\d.,\']*\)?)'
_ANNUAL_TWO_AMOUNTS_RE = re.compile(
    rf'(?P<cur>{_ANNUAL_AMOUNT_TOKEN})\s+(?P<prev>{_ANNUAL_AMOUNT_TOKEN})\s*$'
)
_ANNUAL_ONE_AMOUNT_RE = re.compile(rf'(?P<cur>{_ANNUAL_AMOUNT_TOKEN})\s*$')


def _annual_parse_amount(raw):
    if raw is None:
        return None
    s = str(raw).strip()
    if s in ("", ".", "-", "–", "—"):
    ...
    s = re.sub(r'(?<=\d)[OQ](?=\d|$)', '0', s, flags=re.I)
    s = re.sub(r'[^0-9.,-]', '', s)
    if not re.search(r'\d', s):
        return None
    sign = -1 if s.startswith('-') or neg else 1
    s = s.lstrip('-')
    parts = re.split(r'[.,]', s)
...
def _annual_extract_rows(text):
    """Vrne (normaliziran_naziv, tekoče, preteklo, izvorna_vrstica)."""
    rows = []
    for raw_line in str(text or '').splitlines():
        line = re.sub(r'\s+', ' ', raw_line).strip()
        if not line:
            continue
        # Tesseract včasih navpično črto tabele vrne kot znak '|'; za zneske je to presledek.
        match_line = line.replace('|', ' ')
        m = _ANNUAL_TWO_AMOUNTS_RE.search(match_line)
        if m:
            label = match_line[:m.start()].strip(' :-')
            cur = _annual_parse_amount(m.group('cur'))
            prev = _annual_parse_amount(m.group('prev'))
        else:
            m = _ANNUAL_ONE_AMOUNT_RE.search(match_line)
            if not m:
                continue
            label = match_line[:m.start()].strip(' :-')
            cur = _annual_parse_amount(m.group('cur'))
            prev = None
        norm = _annual_normalize_label(label)
        if norm and cur is not None:
            rows.append((norm, cur, prev, line))
    return rows


# Daljši in bolj specifični nazivi morajo biti pred nadrejenimi postavkami.
_ANNUAL_LABEL_TARGETS = sorted([
    # BS – sredstva
    ("PREDUJMI ZA PRIDOBITEV OPREDMETENIH OSNOVNIH SREDSTEV", "017"),
    ("PREDUJMI ZA PRIDOBITEV OPREDMETENIH", "017"),
    ("OPREDMETENA OSNOVNA SREDSTVA V GRADNJI IN IZDELAVI", "016"),
    ("OPREDMETENA OSNOVNA SREDSTVA V GRADNJI", "016"),
    ("OPREDMETENA OSNOVNA SREDSTVA KI SE PRIDOBIVAJO", "__PPE_ACQUIRED"),
    ("OPREDMETENA OSNOVNA SREDSTVA KI SE", "__PPE_ACQUIRED"),
    ("DOLGOROCNE POSLOVNE TERJATVE DO DRUGIH", "030"),
    ("DOLGOROCNE POSLOVNE TERJATVE", "027"),
    ("NEOPREDMETENA SREDSTVA IN DOLGOROCNE AKTIVNE CASOVNE RAZMEJITVE", "003"),
    ("NEOPREDMETENA SREDSTVA IN DOLGOROCNE AKTIVNE", "003"),
    ("DOLGOROCNE PREMOZENJSKE PRAVICE", "005"),
    ("DRUGE NAPRAVE IN OPREMA", "014"),
    ("OPREDMETENA OSNOVNA SREDSTVA", "010"),
    ("DOLGOROCNA SREDSTVA", "002"),
    ("KRATKOROCNE POSLOVNE TERJATVE DO KUPCEV", "050"),
    ("KRATKOROCNE POSLOVNE TERJATVE DO DRUGIH", "051"),
    ("KRATKOROCNE POSLOVNE TERJATVE", "048"),
    ("KRATKOROCNE AKTIVNE CASOVNE RAZMEJITVE", "053"),
    ("PROIZVODI IN TRGOVSKO BLAGO", "__PRODUCTS_AND_GOODS"),
    ("PREDUJMI ZA ZALOGE", "039"),
    ("KRATKOROCNA SREDSTVA", "032"),
    ("DENARNA SREDSTVA", "052"),
    ("ZEMLJISCA IN ZGRADBE", "012"),
    ("ZGRADBE", "012"),
    ("ZALOGE", "034"),
    ("SREDSTVA", "001"),
    # BS – kapital in obveznosti
    ("OBVEZNOSTI DO VIROV SREDSTEV", "055"),
    ("DOLGOROCNE FINANCNE OBVEZNOSTI DO BANK", "078"),
    ("DOLGOROCNE FINANCNE OBVEZNOSTI", "076"),
    ("DOLGOROCNE OBVEZNOSTI", "075"),
    ("KRATKOROCNE FINANCNE OBVEZNOSTI DO BANK", "089"),
    ("DRUGE KRATKOROCNE FINANCNE OBVEZNOSTI", "090"),
    ("KRATKOROCNE FINANCNE OBVEZNOSTI", "087"),
    ("KRATKOROCNE POSLOVNE OBVEZNOSTI DO DOBAVITELJEV", "093"),
    ("KRATKOROCNE POSLOVNE OBVEZNOSTI NA PODLAGI PREDUJMOV", "__CURRENT_ADVANCES"),
    ("DRUGE KRATKOROCNE POSLOVNE OBVEZNOSTI", "__OTHER_CURRENT_OPERATING"),
    ("KRATKOROCNE POSLOVNE OBVEZNOSTI", "091"),
    ("KRATKOROCNE OBVEZNOSTI", "085"),
    ("CISTI POSLOVNI IZID POSLOVNEGA LETA", "__CURRENT_YEAR_RESULT"),
    ("PRENESENI CISTI POSLOVNI IZID", "__CARRIED_RESULT"),
    ("REZERVE IZ DOBICKA", "061"),
    ("ZAKONSKE REZERVE", "062"),
    ("VPOKLICANI KAPITAL", "057"),
    ("OSNOVNI KAPITAL", "058"),
    ("KAPITAL", "056"),
    # IPI
    ("FINANCNI PRIHODKI IZ POSLOVNIH TERJATEV DO DRUGIH", "165"),
    ("FINANCNI PRIHODKI IZ POSLOVNIH TERJATEV", "163"),
    ("FINANCNI ODHODKI IZ POSOJIL PREJETIH OD BANK", "171"),
    ("FINANCNI ODHODKI IZ DRUGIH FINANCNIH OBVEZNOSTI", "173"),
    ("FINANCNI ODHODKI IZ FINANCNIH OBVEZNOSTI", "169"),
    ("FINANCNI ODHODKI IZ OBVEZNOSTI DO DOBAVITELJEV IN MENICNIH OBVEZNOSTI", "176"),
    ("FINANCNI ODHODKI IZ POSLOVNIH OBVEZNOSTI", "174"),
    ("NABAVNA VREDNOST PRODANEGA BLAGA IN MATERIALA TER STROSKI PORABLJENEGA", "129"),
    ("NABAVNA VREDNOST PRODANIH BLAGA IN MATERIALA TER STROSKI PORABLJENEGA", "129"),
    ("NABAVNA VREDNOST PRODAN", "129"),
    ("STROSKI BLAGA MATERIALA IN STORITEV", "128"),
    ("STROSKI SOCIALNIH ZAVAROVANJ", "__SOCIAL_TOTAL"),
    ("SOCIALNIH ZAVAROVANJ", "__SOCIAL_TOTAL"),
    ("OD TEGA POKOJNINSKIH ZAVAROVANJ", "141"),
    ("POKOJNINSKIH ZAVAROVANJ", "141"),
    ("CISTI POSLOVNI IZID OBRACUNSKEGA OBDOBJA", "__NET_RESULT"),
    ("DRUGI POSLOVNI PRIHODKI", "125"),
    ("DRUGI POSLOVNI ODHODKI", "148"),
    ("CISTI PRIHODKI OD PRODAJE", "110"),
    ("STROSKI STORITEV", "134"),
    ("DRUGI STROSKI DELA", "143"),
    ("STROSKI DELA", "139"),
    ("STROSKI PLAC", "140"),
    ("STROSK PLAC", "140"),
    ("ODPISI VREDNOSTI", "144"),
    ("AMORTIZACIJA", "145"),
    ("DAVEK IZ DOBICKA", "184"),
    ("DRUGI PRIHODKI", "178"),
    ("DRUGI ODHODKI", "181"),
], key=lambda item: len(item[0]), reverse=True)


def _annual_match_target(normalized_label):
    compact = normalized_label.replace(' ', '')
    for alias, target in _ANNUAL_LABEL_TARGETS:
        alias_compact = alias.replace(' ', '')
        if alias_compact not in compact:
            continue
        # Nadrejeni nazivi so kratki in jih pri slabšem OCR ne smemo uporabiti za podpostavko.
        if target == "001" and alias_compact == "SREDSTVA" and compact != "SREDSTVA":
            continue
        if target == "010" and alias_compact == "OPREDMETENAOSNOVNASREDSTVA" and any(
                marker in compact for marker in ("KISE", "VGRADNJI", "PREDUJMI")):
            continue
        if target == "048" and alias_compact == "KRATKOROCNEPOSLOVNETERJATVE" and any(
                marker in compact for marker in ("DOKUPCEV", "DODRUGIH")):
            continue
        if target == "091" and alias_compact == "KRATKOROCNEPOSLOVNEOBVEZNOSTI" and any(
                marker in compact for marker in ("DODOBAVITELJEV", "PREDUJM", "DRUGEKRATKOROCNE")):
            continue
        if target == "056" and alias_compact == "KAPITAL" and any(
                marker in compact for marker in ("VPOKLICANI", "OSNOVNI")):
            continue
...
def _annual_set_entry(result, code, cur, prev=None, overwrite=False):
    if cur is None:
        return
    if overwrite or code not in result.aop_data:
        result.aop_data[code] = AopEntry(aop=code, current_year=float(cur), previous_year=prev)


def _annual_signed_pair(result, signed_value, profit_code, loss_code, gvaop):
    if signed_value is None:
        return
    if signed_value >= 0:
        _annual_set_entry(result, profit_code, signed_value, overwrite=True)
        _annual_set_entry(result, loss_code, 0, overwrite=True)
    else:
        _annual_set_entry(result, profit_code, 0, overwrite=True)
        _annual_set_entry(result, loss_code, abs(signed_value), overwrite=True)
    result.gvaop_data[gvaop] = float(signed_value)


def _annual_company_from_probe(page_texts, statement_year=None):
    c = CompanyInfo()
    all_text = '\n'.join(page_texts[i] for i in sorted(page_texts))
    lines = all_text.splitlines()
    for i, raw in enumerate(lines):
        norm = _annual_normalize_label(raw)
        if not c.name and "NAZIV DRUZBE" in norm:
            candidate = raw.split(':', 1)[1].strip() if ':' in raw else ''
            if not candidate and i + 1 < len(lines):
                candidate = lines[i + 1].strip()
            if candidate:
                candidate = re.sub(r'\bd[\.,]?0[\.,]?0\.?', 'd.o.o.', candidate, flags=re.I)
                c.name = re.sub(r'\s+', ' ', candidate).strip().rstrip(',')
        if not c.registration_number and "MATICNA STEVILKA" in norm:
            nums = re.findall(r'\d{7,10}', raw.replace(' ', ''))
            if nums:
                c.registration_number = nums[-1]
        if not c.tax_number and "DAVCNA STEVILKA" in norm:
            digits = ''.join(re.findall(r'\d', raw))
            if len(digits) >= 8:
                c.tax_number = digits[-8:]
        if not c.period_to and "POSLOVN" in norm and "LET" in norm:
            dates = re.findall(r'\d{1,2}\.\d{1,2}\.\d{4}', raw)
            if len(dates) >= 2:
                c.period_from, c.period_to = dates[0], dates[1]
    if not c.period_to:
        m = re.search(r'(31\.12\.)(20\d{2})', all_text)
        if m:
            c.period_to = m.group(0)
    if not c.period_to and statement_year:
        c.period_to = f"31.12.{statement_year}"
    c.tip_subjekta = "1"
    return c


def parse_ocr_scanned_annual_report(pdf_path, tip_override=None, tip_subjekta_override=None, probe=None):
    result = ParseResult(pdf_format="OCR_ANNUAL")
    if pytesseract is None:
        result.errors.append("PDF je skeniran, knjižnica pytesseract za OCR pa ni nameščena.")
        return result
    probe = probe or _probe_scanned_pdf(pdf_path)
    required = (probe.get("bs_assets"), probe.get("bs_liabilities"), probe.get("ipi"))
    if any(pi is None for pi in required):
        result.errors.append(
            "V skeniranem letnem poročilu niso bile najdene vse tri tabele: sredstva, "
            "obveznosti do virov sredstev in izkaz poslovnega izida."
        )
        return result

    page_text = {}
    with pdfplumber.open(pdf_path) as pdf:
        for key in ("bs_assets", "bs_liabilities", "ipi"):
            pi = probe[key]
            page_text[key] = _annual_ocr_page_text(pdf.pages[pi], dpi=180, psm=6)

    # Leto iz glave IPI je zanesljiv rezervni podatek, če predstavitvena stran nima obdobja.
    years = re.findall(r'\b20\d{2}\b', page_text["ipi"])
    statement_year = years[0] if years else None
    c = _annual_company_from_probe(probe.get("page_texts", {}), statement_year)
    result.company = finalize_company(c, tip_override, tip_subjekta_override)
    result.subject_type = _subject_type_from_tip_subjekta(result.company.tip_subjekta)

    temporary = {}
    matched_lines = 0
    for section in ("bs_assets", "bs_liabilities", "ipi"):
        for label, cur, prev, raw in _annual_extract_rows(page_text[section]):
            target = _annual_match_target(label)
            if not target:
                continue
            matched_lines += 1
            if target.startswith("__"):
                temporary[target] = (cur, prev, raw)
            else:
                _annual_set_entry(result, target, cur, prev)

    def _cur(code):
        entry = result.aop_data.get(code)
        return entry.current_year if entry and entry.current_year is not None else 0.0

    # Podrejene vrstice, ki jih sken pogosto prebere slabše kot njihove kontrolne seštevke.
    if "048" in result.aop_data and "050" in result.aop_data:
        _annual_set_entry(result, "051", _cur("048") - _cur("049") - _cur("050"), overwrite=True)

    # PPE predujmi: če OCR zamenja 1↔4, uporabimo izrecni vmesni seštevek iz poročila.
    acquired = temporary.get("__PPE_ACQUIRED")
    if acquired and "016" in result.aop_data:
        derived_017 = acquired[0] - _cur("016")
        if derived_017 >= 0:
            old_017 = _cur("017") if "017" in result.aop_data else None
            if old_017 is None or abs(old_017 - derived_017) > 0.02:
                _annual_set_entry(result, "017", derived_017, overwrite=True)
                result.warnings.append(
                    "AOP 017 je bil popravljen z bilančno kontrolo: sredstva v pridobivanju "
                    "minus sredstva v gradnji/izdelavi."
                )

    # Druge kratkoročne poslovne obveznosti v CBK vključujejo predujme in ostale obveznosti.
    adv = temporary.get("__CURRENT_ADVANCES")
    other = temporary.get("__OTHER_CURRENT_OPERATING")
    if "091" in result.aop_data and "093" in result.aop_data:
        _annual_set_entry(result, "094", _cur("091") - _cur("093"), overwrite=True)
    elif adv or other:
        _annual_set_entry(result, "094", (adv[0] if adv else 0) + (other[0] if other else 0), overwrite=True)

    # Če je izkazana samo ena vrsta dolgoročnih finančnih obveznosti, je bančna podpostavka
    # enaka nadrejeni vrstici. Enako velja za finančne prihodke iz poslovnih terjatev do drugih.
    if "076" in result.aop_data and "078" not in result.aop_data:
        _annual_set_entry(result, "078", _cur("076"), overwrite=True)
    if "163" in result.aop_data and "165" not in result.aop_data:
        _annual_set_entry(result, "165", _cur("163"), overwrite=True)
    if "169" in result.aop_data and "173" in result.aop_data and "171" not in result.aop_data:
        bank_interest = _cur("169") - _cur("170") - _cur("172") - _cur("173")
        if bank_interest >= 0:
            _annual_set_entry(result, "171", bank_interest, overwrite=True)

    carried = temporary.get("__CARRIED_RESULT")
    if carried:
        _annual_signed_pair(result, carried[0], "068", "069", "0030104")
    current_year_result = temporary.get("__CURRENT_YEAR_RESULT")
    if current_year_result:
        _annual_signed_pair(result, current_year_result[0], "070", "071", "0030105")

    social = temporary.get("__SOCIAL_TOTAL")
    if social and "141" in result.aop_data:
        other_social = social[0] - _cur("141")
        if other_social >= 0:
            _annual_set_entry(result, "142", other_social, overwrite=True)

    # V poročilu sta nabavna vrednost in porabljeni material združena; vnesemo ju v AOP 129,
    # AOP 130 pa ostane 0. Kontrolni izračun iz AOP 128 in stroškov storitev je natančnejši od
    # neposrednega OCR-a dolge, drobno tiskane vrstice.
    if "128" in result.aop_data and "134" in result.aop_data:
        goods_material = _cur("128") - _cur("134")
        if goods_material >= 0:
            _annual_set_entry(result, "129", goods_material, overwrite=True)
    if social and "139" in result.aop_data and "143" in result.aop_data:
        wages = _cur("139") - social[0] - _cur("143")
        if wages >= 0:
            _annual_set_entry(result, "140", wages, overwrite=True)
    if "144" in result.aop_data and not (_cur("146") or _cur("147")):
        _annual_set_entry(result, "145", _cur("144"), overwrite=True)

    # IPI zbirne vrstice in dobiček/izguba iz poslovanja morajo biti izrecno zapisani v Excel.
    gross = _cur("110") + _cur("121") - _cur("122") + _cur("123") + _cur("124") + _cur("125")
    operating_expenses = _cur("128") + _cur("139") + _cur("144") + _cur("148")
    _annual_set_entry(result, "126", gross, overwrite=True)
    _annual_set_entry(result, "127", operating_expenses, overwrite=True)
    operating_result = gross - operating_expenses
    _annual_signed_pair(result, operating_result, "151", "152", "061")

    financial_income = _cur("155") + _cur("160") + _cur("163")
    _annual_set_entry(result, "153", financial_income, overwrite=True)
    net = temporary.get("__NET_RESULT")
    net_value = net[0] if net else None

    # Če je neto rezultat v izkazu izrecno naveden, iz njega in davka dobimo najbolj zanesljiv
    # celotni rezultat. Nato z isto enačbo preverimo/skorigiramo skupne finančne odhodke.
    if net_value is not None:
        pretax = net_value + _cur("184") + _cur("185")
        derived_fin_exp = operating_result + financial_income + _cur("178") - _cur("181") - pretax
        if derived_fin_exp >= 0:
            _annual_set_entry(result, "166", derived_fin_exp, overwrite=True)
            derived_operating_fin = derived_fin_exp - _cur("168") - _cur("169")
            if derived_operating_fin >= 0:
                if "174" not in result.aop_data or abs(_cur("174") - derived_operating_fin) > 1.02:
                    _annual_set_entry(result, "174", derived_operating_fin, overwrite=True)
                if "176" in result.aop_data and abs(_cur("176") - _cur("174")) <= 1.02:
                    _annual_set_entry(result, "176", _cur("174"), overwrite=True)
        else:
            financial_expenses = _cur("168") + _cur("169") + _cur("174")
            _annual_set_entry(result, "166", financial_expenses, overwrite=True)
    else:
        financial_expenses = _cur("168") + _cur("169") + _cur("174")
        _annual_set_entry(result, "166", financial_expenses, overwrite=True)
        pretax = operating_result + financial_income - financial_expenses + _cur("178") - _cur("181")
        net_value = pretax - _cur("184") - _cur("185")

    _annual_signed_pair(result, pretax, "182", "183", "071")
    _annual_signed_pair(result, net_value, "186", "187", "075")
    # Neto rezultat IPI je hkrati rezultat poslovnega leta v kapitalu BS.
    _annual_signed_pair(result, net_value, "070", "071", "0030105")

    # Strežniške različice Tesseracta včasih zgrešijo samo kratko krepko vrstico SREDSTVA
    # ali OBVEZNOSTI DO VIROV SREDSTEV, čeprav vse podrejene skupine preberejo pravilno.
    # Tak izpad ne sme blokirati izvoza: manjkajoči kontrolni seštevek obnovimo iz skupin.
    if "001" not in result.aop_data:
        if "002" in result.aop_data and "032" in result.aop_data:
            assets_total = _cur("002") + _cur("032") + _cur("053")
            _annual_set_entry(result, "001", assets_total, overwrite=True)
            result.warnings.append("AOP 001 je bil obnovljen iz AOP 002 + 032 + 053.")
        elif "055" in result.aop_data:
            _annual_set_entry(result, "001", _cur("055"), overwrite=True)
            result.warnings.append("AOP 001 je bil obnovljen iz uravnotežene pasive AOP 055.")
    if "055" not in result.aop_data:
        if "056" in result.aop_data and "075" in result.aop_data and "085" in result.aop_data:
            sources_total = _cur("056") + _cur("072") + _cur("075") + _cur("085") + _cur("095")
            _annual_set_entry(result, "055", sources_total, overwrite=True)
            result.warnings.append("AOP 055 je bil obnovljen iz skupin kapitala in obveznosti.")
        elif "001" in result.aop_data:
            _annual_set_entry(result, "055", _cur("001"), overwrite=True)
            result.warnings.append("AOP 055 je bil obnovljen iz uravnotežene aktive AOP 001.")

    # Kontrole: večja odstopanja naj bodo vidna, vendar zaradi enega OCR dvoma ne blokiramo izvoza.
    if "001" in result.aop_data and "055" in result.aop_data and abs(_cur("001") - _cur("055")) > 0.02:
        result.warnings.append("Kontrola BS ni uspela: AOP 001 ni enak AOP 055.")
    if abs((pretax - _cur("184") - _cur("185")) - net_value) > 1.02:
        result.warnings.append("Kontrola IPI ni uspela: izračunani in izkazani čisti poslovni izid se razlikujeta.")
    if temporary.get("__PRODUCTS_AND_GOODS"):
        result.warnings.append(
            "Poročilo združuje proizvode in trgovsko blago. AOP 034 je vnesen, AOP 037/038 pa "
            "ostajata nerazdeljena, ker izvor ne vsebuje ločenih zneskov."
        )
    if not result.company.name or not result.company.registration_number:
        result.warnings.append("Ime podjetja in/ali matična številka z OCR nista bila zanesljivo prepoznana.")
    result.warnings.append(
        f"Skenirano letno poročilo brez AOP oznak: uporabljene strani PDF "
        f"{probe['bs_assets'] + 1}, {probe['bs_liabilities'] + 1} in {probe['ipi'] + 1}; "
        f"prepoznanih {matched_lines} podatkovnih vrstic. Pred oddajo primerjaj ključne zneske z izvirnikom."
    )
    return validate(result)
...
def parse_pdf_file(pdf_path, tip_override=None, tip_subjekta_override=None):
    ...
    if is_scanned_no_text(pages_text):
        probe = _probe_scanned_pdf(pdf_path)
        if probe.get("kind") == "OCR_ANNUAL":
            return parse_ocr_scanned_annual_report(
                pdf_path, tip_override, tip_subjekta_override, probe=probe
            )
...
def export_excel(result, output_path):
    ...
    Prejšnja verzija je nastavljala number_format in datume pretvarjala v Excel date objekte,
    kar lahko pri uvozu v bančni/Oracle program povzroči ORA-01722 invalid number.
    Zato tukaj samo kopiramo template in vpišemo vrednosti v obstoječe celice,
    formatiranje, validacije in tipi celic iz template-a pa ostanejo nespremenjeni.
    """
    ...
    # Ne pretvarjamo v Excel date objekt; pustimo format/vrednost kot tekst po obstoječem template-u.
    ...
    if meseci is not None:
        result.aop_data["189"] = AopEntry(aop="189", current_year=float(meseci), previous_year=None)

    for code in IPI_AOP:
        if code not in idx2 or code not in result.aop_data:
            continue
        val = result.aop_data[code].current_year
        if val is not None:
            ws2.cell(idx2[code],4).value = val
            ipi_n += 1
    ipi_n += _fill_combined_rows(ws2, result.aop_data, _COMBINED_ROWS_IPI)
    if result.gvaop_data:
  ...
  <div class="card">
    ...
    <div id="app-section" style="display:none;">
      ...
      <div id="medletni-datum-field" style="display:none;">
        <label class="field-label" for="medletni-mesec-select">Na kateri datum je medletna bilanca narejena?</label>
        <input type="month" id="medletni-mesec-select" onchange="selectMedletniMesec(this.value)">
...
function selectMedletniMesec(val){ selectedMedletniMesec=val; }
...
function toggleMedletniDatumField(){
  ...
  if(selectedTip==='7'){
    ...
    document.getElementById('medletni-mesec-select').value='';
  }
}
