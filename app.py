...
def validate(result):
    if not result.company.name and result.pdf_format not in ("GDZADRUGE", "OCR"):
        result.errors.append("Ime podjetja ni najdeno.")
    if not result.company.registration_number and result.pdf_format not in ("UPR", "GDZADRUGE", "OCR", "POROCILO_GD"):
    ...
    if (has_bs or not has_ipi) and result.pdf_format != "OCR":
        for code in ["001","055"]:
...
def parse_pdf_file(pdf_path, tip_override=None, tip_subjekta_override=None):
    ...
    # Preveri ali je PDF CID-encoded (pokvarjen font) - besedila ni mogoče prebrati normalno,
    # zato poskusimo z OCR (prepoznavanje iz slike strani) kot zadnjo možnostjo.
